...
- [x] Use authelia as the forward auth for the traefik dashboard.
- [x] We need to do a full audit our cluster. How much memory are we using versus how much we are allocating? We have limited resources and right now we don't even have any actual applications running. We want to make sure our platform services are resilient, but we also want to maintain efficiency.
...
- [x] Fix Prometheus disk space issue. Increased Prometheus from 3Gi→8Gi (now at 44% usage). Registry kept at 15Gi.
